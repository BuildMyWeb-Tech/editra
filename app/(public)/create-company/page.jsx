...
export default function CreateStore() {
    ...
    const fetchSellerStatus = async () => {
        ...
        try {
            ...
            if (['approved', 'rejected', 'pending'].includes(data.status)) {
                ...
                switch (data.status) {
                    case "approved":
                        setMessage("Your company request has been approved. You can now post jobs from your dashboard.")
                        setTimeout(() => router.push("/store"), 5000)
                        break;
                    case "rejected":
                        setMessage("Your company request has been rejected. Contact the admin for more details.")
                        break;
                    case "pending":
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            router.push("/company/waiting")   // create this page            
            ...
            {!alreadySubmitted ? (
                <div className="mx-6 min-h-[70vh] my-16">
                    <form onSubmit={e => toast.promise(onSubmitHandler(e), { loading: "Submitting data..." })} className="max-w-7xl mx-auto flex flex-col items-start gap-3 text-slate-500">
                        {/* Title */}
                        <div>
                            <h1 className="text-3xl ">Add Your <span className="text-slate-800 font-medium">Company</span></h1>
                            <p className="max-w-lg">To post jobs, submit your company details for review. Your company will be activated after admin verification.</p>
                        </div>
                        ...
                        {/* <p>Website</p>
                        <input name="website" onChange={onChangeHandler} value={storeInfo.website} type="text" placeholder="Enter your company website URL" className="border border-slate-300 outline-slate-400 w-full max-w-lg p-2 rounded" /> */}
